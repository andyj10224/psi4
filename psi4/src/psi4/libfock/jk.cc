...
std::shared_ptr<JK> JK::build_JK(std::shared_ptr<BasisSet> primary, std::shared_ptr<BasisSet> auxiliary,
    ...
    if (jk_type == "DF") {
        ...
        return std::shared_ptr<JK>(jk);
    } else if (jk_type == "DF_LINEAR") {
        CompositeJK* jk = new CompositeJK(primary, auxiliary, "DFCFMM", "LINK", options);
        // Composite JK algorithms should use Incremental Fock build by default
        if (!options["INCFOCK"].has_changed()) jk->set_incfock(true);

        return std::shared_ptr<JK>(jk);
    } else if (jk_type == "LOCAL_DF") {
        CompositeJK* jk = new CompositeJK(primary, auxiliary, "LOCAL_DF", "LINK", options);
        // Composite JK algorithms should use Incremental Fock build by default
        if (!options["INCFOCK"].has_changed()) jk->set_incfock(true);

        return std::shared_ptr<JK>(jk);
    } else if (jk_type == "LOCAL_DF2") {
        CompositeJK* jk = new CompositeJK(primary, auxiliary, "LOCAL_DF", "LOCAL_DF", options);
        // Composite JK algorithms should use Incremental Fock build by default
        if (!options["INCFOCK"].has_changed()) jk->set_incfock(true);
