...
CGHF::CGHF(SharedWavefunction ref_wfn, std::shared_ptr<SuperFunctional> func, Options& options, std::shared_ptr<PSIO> psio)
...
void CGHF::common_init() {
    ...
    // Right now, Psi4 assumes that these quantities exist,
    // if they are NOT initialized in the HF subclass constructor
    ...
    G_mat = mintshelper()->ao_eri(); // BAD!!!!! THIS IS O(N^{4}) STORAGE !!!!!!
    // Personal Foul, unnecessary memory! 15-yard penalty!
