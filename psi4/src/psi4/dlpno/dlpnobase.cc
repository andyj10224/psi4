...
void DLPNOBase::common_init() {
    ...
    T_CUT_PNO_SINGLES_ = options_.get_double("T_CUT_PNO_SINGLES");
    ...
    const bool T_CUT_PNO_SINGLES_changed = options_["T_CUT_PNO_SINGLES"].has_changed();
    ...
    if (algorithm_ == MP2) {
        ...
        if (options_.get_str("PNO_CONVERGENCE") == "LOOSE") {
            ...
            if (!T_CUT_DO_changed) T_CUT_DO_ = 2e-2;
            if (!T_CUT_PNO_SINGLES_changed) T_CUT_PNO_SINGLES_ = 3e-8;
            ...
            if (!T_CUT_DO_changed) T_CUT_DO_ = 1e-2;
            if (!T_CUT_PNO_SINGLES_changed) T_CUT_PNO_SINGLES_ = 1e-8;
            ...
            if (!T_CUT_PNO_changed) T_CUT_PNO_ = 1e-8;
            if (!T_CUT_DO_changed) T_CUT_DO_ = 5e-3;
            if (!T_CUT_PNO_SINGLES_changed) T_CUT_PNO_SINGLES_ = 1e-12;
...
void DLPNOBase::prep_sparsity(bool initial, bool last) {
    int natom = molecule_->natom();
    int nbf = basisset_->nbf();
    int nshell = basisset_->nshell();
    int naux = ribasis_->nbf();
